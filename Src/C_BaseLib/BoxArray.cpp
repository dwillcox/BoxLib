...
BoxLib::intersect (const BoxArray& lhs,
		   const BoxArray& rhs)
{
    const int Nlhs = lhs.size();
    const int Nrhs = rhs.size();

    if (Nlhs == 0 || Nrhs == 0) return BoxArray();

    BoxList bl(lhs.ixType());
			 ...
			 bool first_only) const
{
...
#endif
    {
	    ...
	    for (int i = 1; i < N; ++i)
            {
                const Box& nbx = BoxLib::surroundingNodes(m_ref->m_abox[i]);
                boundingbox.minBox(nbx);

                maxext = BoxLib::max(maxext, nbx.size());
            }
